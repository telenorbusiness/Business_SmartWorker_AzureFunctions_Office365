...
const promiseReferenceTokenClient = Issuer
  .discover( process.env['idpUrl'] )
  .then((issuer) => {
    let keyString = new Buffer(process.env['authKey'], 'base64').toString();
    return jose.JWK.asKey(keyString, "json")
      .then(key => {
...
let authenticateReferenceToken = function ( req ){
  return promiseReferenceTokenClient
    .then((client) => {
      if( lodash.trim(lodash.get(req, "headers.authorization", "").replace(/(?:(B|b)earer )/,"")) === ""  ){
          ...
          .then((userinfo) => {
            if( userinfo.success === true){
              if(!userinfo.azureUserToken) {
                return Promise.resolve({ status: 400, message: "Missing azuretoken" });
              }
              return Promise.resolve({ status: 200, azureUserToken: userinfo.azureUserToken, sub: userinfo.sub });
            }
            else if( !lodash.isUndefined(userinfo.error) ){
              return Promise.resolve({ status: 200, message: userinfo });
            }
            else {
              throw new Error(`This is not a reference token: ${JSON.stringify(userinfo)}`);
            }
          });
      }
    })
