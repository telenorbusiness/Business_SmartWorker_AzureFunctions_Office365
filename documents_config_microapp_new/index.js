    ...
    .then(response => {
      if (response.status === 200 && !response.message) {
        if (checkIsAdminFromSmartAnsatt(response) && response.configId) {
          return getStorageInfo(response.configId, context).then(
            defaultValue => {
              const res = {
                body: createSearchMicroApp(
                  response.configId,
                  defaultValue,
...
function createSearchMicroApp(configId, defaultValue = null, graphToken, search = "") {
